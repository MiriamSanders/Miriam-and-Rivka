...
//   const dishTypeClause = dishType && dishType.length > 1 ? 'AND r.dishType LIKE ?' : '';
...
//       ${excludeSQL}
//     GROUP BY r.recipeId, r.title, r.category, r.dishType
//     HAVING (MAX(ur.rating) > 3 OR (MAX(ur.rating) = 0 AND AVG(rr.rating) > 3) OR (MAX(ur.rating) = 0 AND AVG(rr.rating) = 0))
//     ORDER BY CASE 
...
//   if (dishTypeClause) params.push(`%${dishType}%`);
//   if (excludeIds.length) params.push(...excludeIds);

//   const [rows] = await db.execute(sql, params);
//   console.log(`Fetched ${rows.length} backups for category="${preferredCategory}", dishType="${dishType}"`);
//   return rows.map(r => ({ ...r, recipeId: String(r.recipeId) }));
// }
...
//     const backups = await getBackupRecipes(preferredCat, dishType, userId, allOrigIds);
...
//       if (side) replacements.push({ day, course: 'side', reason: 'insufficient original recipes' });
//     }
//     if (!main) {
//       const preferredCat = allMains.length > 0 ? allMains[0].category : 'Parve';
//       main = await chooseBackup(preferredCat, 'main');
//       if (main) replacements.push({ day, course: 'main', reason: 'insufficient original recipes' });
//     }
//     if (!dessert) {
//       const preferredCat = allDesserts.length > 0 ? allDesserts[0].category : 'Parve';
//       dessert = await chooseBackup(preferredCat, 'dessert');
//       if (dessert) replacements.push({ day, course: 'dessert', reason: 'insufficient original recipes' });
//     }
...
//HAVING (MAX(ur.rating) > 3 OR (MAX(ur.rating) = 0 AND AVG(rr.rating) > 3) OR (MAX(ur.rating) = 0 AND AVG(rr.rating) = 0)) - line 360
  ...
  // // If no results, try without dishType filter
  // if (rows.length === 0 && dishType) {
  //   console.log(`Trying again without dishType filter...`);
  //   sql = `
  //     SELECT  r.recipeId, r.title, r.category, r.dishType,
  //             COALESCE(AVG(rr.rating),0) AS avgRating,
  //             COALESCE(MAX(ur.rating),0) AS userRating,
  ...
  //   params = [userId, preferredCategory];
  //   if (excludeIds.length) params.push(...excludeIds);
  //   [rows] = await db.execute(sql, params);
  //   console.log(`Second attempt: Found ${rows.length} backups without dishType filter`);
  // }

  // // If still no results, try with any category but lower rating threshold
  // if (rows.length === 0) {
  //   console.log(`Trying with any category and lower rating threshold...`);
  //   sql = `
  //     SELECT  r.recipeId, r.title, r.category, r.dishType,
  //             COALESCE(AVG(rr.rating),0) AS avgRating,
  //             COALESCE(MAX(ur.rating),0) AS userRating,
  ...
  //   if (excludeIds.length) params.push(...excludeIds);
  //   [rows] = await db.execute(sql, params);
  //   console.log(`Third attempt: Found ${rows.length} backups with any category`);
  // }
...
//     console.log(`Available from original: side=${!!side}, main=${!!main}, dessert=${!!dessert}`);
...
//           dessert = await chooseBackup('Parve', 'dessert');
//           if (dessert) replacements.push({ day, course: 'dessert', reason: 'kosher compliance - replaced dairy with parve' });
//         } else if (side.category === 'Dairy') {
//           console.log(`Replacing dairy side with parve...`);
//           side = await chooseBackup('Parve', 'side');
//           if (side) replacements.push({ day, course: 'side', reason: 'kosher compliance - replaced dairy with parve' });
//         } else if (main.category === 'Dairy') {
//           console.log(`Replacing dairy main with parve...`);
//           main = await chooseBackup('Parve', 'main');
//           if (main) replacements.push({ day, course: 'main', reason: 'kosher compliance - replaced dairy with parve' });
//         }
//       }
//     }
...
//       side = await chooseBackup(preferredCat, 'side');
//       if (side) {
//         console.log(`Found backup side: ${side.title} (${side.category})`);
...
//       main = await chooseBackup(preferredCat, 'main');
//       if (main) {
//         console.log(`Found backup main: ${main.title} (${main.category})`);
...
//       dessert = await chooseBackup(preferredCat, 'dessert');
//       if (dessert) {
//         console.log(`Found backup dessert: ${dessert.title} (${dessert.category})`);
